...
bool ESP8266::sATCIPCLOSEMulitple(uint8_t mux_id)
{
    ...
    data = recvString("OK", "UNLINK", 5000);
  
    if (data.indexOf("OK") != -1 || data.indexOf("UNLINK") != -1) {
        return true;
    }
    return false;
}
...
bool ESP8266::setStationIp(String ip, String gateway, String netmask)
{
    rx_empty();
    m_puart->println("AT+CIPSTA_CUR=\"" + ip + "\",\"" + gateway + "\",\"" + netmask + "\"");
    return recvFind("OK");
}